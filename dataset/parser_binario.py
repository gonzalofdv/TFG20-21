...
#CSV salida
csvObj = open("dataset_binario.csv", "w", encoding="utf-8")
...
for item in diccionario2:
	...
	resultado2.append(0) #porque es goodware
...
csvObj.close()
dataset = pd.read_csv("dataset_binario.csv")
...
dataset5 = dataset4[dataset4['label']==1].sample(n=357) #357 filas ransomware aleatorias
dataset6 = dataset4[dataset4['label']==0]
dataset7 = pd.concat([dataset5, dataset6], axis=0)
print("----------------------------------------------")
print("Reparto final del dataset: ")
print(dataset7['label'].value_counts()) #Print reparto final
dataset7.to_csv("dataset_binario.csv")
